...
class Board:
    ...
    def add_piece(self, row, col, player):
        if self.valid_move(row, col, player):
            # Add the current state to the stack before making a move
            self.add_prev_state(self.get_board())
    ...
    def undo_move(self):
        if not self.prev_states.is_empty():
            prev_state = self.prev_states.pop()
            self.set(prev_state)
            self.turn -= 1  # Adjust the turn count accordingly
            return True
        return False

    def add_prev_state(self, state):
        # Add current board state to the stack
        self.prev_states.push(state)
...
# Modify the undo_move function
def undo_move():
    global board, status, current_player, has_winner, overflowing, numsteps
    if board.undo_move():
        current_player = (current_player - 1) % 2  # Switch back to the previous player
        numsteps -= 1
        print("Undo move successful")
    else:
        print("Cannot undo move, invalid state. Numsteps:", numsteps)
        ...
        if event.type == pygame.QUIT:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if undo_button.is_clicked(event):
                    print("Undo button clicked")  # Debugging statement
