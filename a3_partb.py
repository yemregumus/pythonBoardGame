...
class GameTree:
    ...
    def build_tree(self, node):
        ...
        if node.depth == node.tree_height - 1 or has_winning_config(node.board, 1) or has_winning_config(node.board, -1):
        ...
        for col in range(len(node.board[0])):
            if not overflow(node.board, col):
                ...
                new_board = overflow(new_board, col)  # update the new board with the result of overflow
                child_node = self.Node(new_board, node.depth + 1, -node.player, node.tree_height, move=(node.depth, col))
        ...
        if node.children:
            ...
            if all(score is None for score in child_scores):
                ...
                if node.depth % 2 == 0:
                    ...
                    node.score = max(score for score in child_scores if score is not None)
                else:
                    # Minimize for odd-depth nodes
                    node.score = min(score for score in child_scores if score is not None)
    ...
    def get_move(self):
        ...
        valid_children = [child for child in self.root.children if child.score is not None and child.move is not None]
