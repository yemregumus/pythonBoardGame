...
class Stack:
	...
	def capacity(self): return self.capp
	...
	def push(self, data):
		...
		if self.occupied == self.capp:
			...
			for i in range(self.capp): new_list[i] = self.items[i]
	...
	def pop(self):
		...
		if self.is_empty(): raise IndexError('pop() used on empty stack')
		else:
	...
	def get_top(self):
		...
		else: return self.items[self.occupied - 1]

	def is_empty(self): return self.occupied == 0

	def __len__(self): return self.occupied
...
class Queue:
	...
	def enqueue(self, data):
		...
		if self.occupied == self.capp:
			...
			for i in range(self.capp): new_list[i] = self.items[(self.front + i) % self.capp]
	...
	def dequeue(self):
		...
		else: raise IndexError('dequeue() used on empty queue')

	def get_front(self):

		if not self.is_empty(): return self.items[self.front]
		else: return None
...
class Deque:
	...
	def push_front(self, data):
		...
		if not self.is_empty(): self.front = (self.front - 1) % self.capp
	...
	def push_back(self, data):
		...
		if not self.is_empty(): self.rear = (self.rear + 1) % self.capp
	...
	def pop_front(self):
		...
		else: raise IndexError('pop_front() used on empty deque')
	...
	def pop_back(self):
		...
		else: raise IndexError('pop_back() used on empty deque')
	...
	def get_back(self):
		...
		if not self.is_empty(): return self.items[self.rear]
		else: return None
	...
	def __getitem__(self, k):
		...
		if 0 <= k < self.occupied: return self.items[(self.front + k) % self.capp]
